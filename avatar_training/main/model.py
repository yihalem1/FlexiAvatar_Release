...
class Model(nn.Module):
    def __init__(self, scene_gaussian, human_gaussian, smplx_param_dict):
        ...
        assert cfg.num_gpus == 1, \
            'visibility buffers are updated inside forward(); DataParallel replicas would discard them'
        self.human_gaussian.set_mesh_neighbors(self.lap_reg.neighbor_idxs,
                                               self.lap_reg.neighbor_weights)

    def slice_for_render(self, asset):
     
        keys = ('mean_3d', 'opacity', 'scale', 'rotation', 'rgb')
        idx = asset.get('render_idx', None)
        if idx is None:
            return {k: asset[k] for k in keys}, None
        return {k: asset[k][idx] for k in keys}, idx
    ...
    def forward(self, data, cur_itr, mode):
        ...
        use_vis = not cfg.fit_pose_to_test
        if use_vis and (mode == 'train') and (cur_itr >= cfg.vis_warmup_itr):
            hg = self.human_gaussian
            first = hg.vis_ready.item() == 0
            if first or ((cur_itr - cfg.vis_warmup_itr) % visibility.REFRESH_ITR == 0):
                hg.update_keep_mask(cur_itr)
        ...
        for i in range(batch_size):
            ...
            human_asset, human_asset_refined, human_offset, mesh_neutral_pose = self.human_gaussian(
                smplx_param, {k: v[i] for k,v in data['cam_param'].items()},
                vis_itr=(cur_itr if mode == 'train' else None), apply_visibility=use_vis)

            key_list = ['mean_3d', 'scale', 'rotation', 'rgb', 'active_mask']
            ...
            human_view, human_idx = self.slice_for_render(human_asset)
            human_view_refined, human_idx_refined = self.slice_for_render(human_asset_refined)
            ...
            for key in ['mean_3d', 'opacity', 'scale', 'rotation', 'rgb']:
                scene_human_asset[key] = torch.cat((scene_asset[key].detach(), human_view[key])) # detach scene

            # combine scene and human assets (refined)
            scene_human_asset_refined = {}
            for key in ['mean_3d', 'opacity', 'scale', 'rotation', 'rgb']:
                scene_human_asset_refined[key] = torch.cat((scene_asset[key].detach(), human_view_refined[key])) # detach scene
            ...
            # human render
            human_render = self.gaussian_renderer(human_view, (img_height, img_width), {k: v[i] for k,v in data['cam_param'].items()}, bg)
            for key in ['img', 'mask']:
            ...
            human_render_refined = self.gaussian_renderer(human_view_refined, (img_height, img_width), {k: v[i] for k,v in data['cam_param'].items()}, bg)
            for key in ['img', 'mask']:
            ...
            if use_vis and (mode == 'train'):
                self.human_gaussian.accumulate_visibility(
                    human_render_refined['is_vis'], human_idx_refined,
                    human_asset_refined['mean_3d'],
                    {k: v[i] for k,v in data['cam_param'].items()},
                    (img_height, img_width), data['bbox'][i],
                    is_generated=bool(int(data['source'][i])) if 'source' in data else False)
        ...
        if mode == 'train':
            ...
            is_gen = data['source'].view(-1).cuda() > 0 if 'source' in data else torch.zeros(0).cuda() > 0
            gen_idx = torch.nonzero(is_gen, as_tuple=True)[0]
            cap_idx = torch.nonzero(~is_gen, as_tuple=True)[0] if is_gen.numel() > 0 else torch.arange(data['img'].shape[0]).cuda()

            # loss functions
            loss = {}
            if cap_idx.numel() > 0:
                cap_img, cap_mask, cap_bbox = data['img'][cap_idx], data['mask'][cap_idx], data['bbox'][cap_idx]
                loss['rgb_human'] = self.rgb_loss(scene_human_renders['img'][cap_idx], cap_img, bbox=cap_bbox) * cfg.rgb_loss_weight
                loss['ssim_human'] = (1 - self.ssim(scene_human_renders['img'][cap_idx], cap_img, bbox=cap_bbox)) * cfg.ssim_loss_weight
                loss['lpips_human'] = self.lpips(scene_human_renders['img'][cap_idx], cap_img, bbox=cap_bbox) * cfg.lpips_weight
                is_face = ((face_renders[cap_idx][:,:3] != -1) * (face_renders[cap_idx][:,3:] == 1)).float()
                loss['rgb_face'] = self.rgb_loss(scene_human_renders['img'][cap_idx] * (1 - is_face) + face_renders[cap_idx][:,:3] * is_face, cap_img, bbox=cap_bbox) * cfg.rgb_loss_weight
                loss['rgb_human_rand_bg'] = self.rgb_loss(human_renders['img'][cap_idx], cap_img, bbox=cap_bbox, mask=cap_mask, bg=bg[None])

                loss['rgb_human_refined'] = self.rgb_loss(scene_human_renders_refined['img'][cap_idx], cap_img, bbox=cap_bbox) * cfg.rgb_loss_weight
                loss['ssim_human_refined'] = (1 - self.ssim(scene_human_renders_refined['img'][cap_idx], cap_img, bbox=cap_bbox)) * cfg.ssim_loss_weight
                loss['lpips_human_refined'] = self.lpips(scene_human_renders_refined['img'][cap_idx], cap_img, bbox=cap_bbox) * cfg.lpips_weight
                is_face = ((face_renders_refined[cap_idx][:,:3] != -1) * (face_renders_refined[cap_idx][:,3:] == 1)).float()
                loss['rgb_face_refined'] = self.rgb_loss(scene_human_renders_refined['img'][cap_idx] * (1 - is_face) + face_renders_refined[cap_idx][:,:3] * is_face, cap_img, bbox=cap_bbox) * cfg.rgb_loss_weight
                loss['rgb_human_refined_rand_bg'] = self.rgb_loss(human_renders_refined['img'][cap_idx], cap_img, bbox=cap_bbox, mask=cap_mask, bg=bg[None])

            if gen_idx.numel() > 0:
                gen_img, gen_mask, gen_bbox = data['img'][gen_idx], data['mask'][gen_idx], data['bbox'][gen_idx]
                gen_gt = gen_img * gen_mask + (1 - gen_mask) * bg[None][:,:,None,None]
                loss['rgb_human_gen'] = self.rgb_loss(human_renders['img'][gen_idx], gen_gt, bbox=gen_bbox) * cfg.gen_rgb_loss_weight
                loss['ssim_human_gen'] = (1 - self.ssim(human_renders['img'][gen_idx], gen_gt, bbox=gen_bbox)) * cfg.gen_ssim_loss_weight
                loss['lpips_human_gen'] = self.lpips(human_renders['img'][gen_idx], gen_gt, bbox=gen_bbox) * cfg.gen_lpips_loss_weight
                loss['rgb_human_refined_gen'] = self.rgb_loss(human_renders_refined['img'][gen_idx], gen_gt, bbox=gen_bbox) * cfg.gen_rgb_loss_weight
                loss['ssim_human_refined_gen'] = (1 - self.ssim(human_renders_refined['img'][gen_idx], gen_gt, bbox=gen_bbox)) * cfg.gen_ssim_loss_weight
                loss['lpips_human_refined_gen'] = self.lpips(human_renders_refined['img'][gen_idx], gen_gt, bbox=gen_bbox) * cfg.gen_lpips_loss_weight

            if cfg.fit_pose_to_test:
                return stats, loss

            if cap_idx.numel() > 0:
                loss['rgb_scene'] = self.rgb_loss(scene_renders['img'][cap_idx], data['img'][cap_idx]) * (1 - data['mask'][cap_idx]) * cfg.rgb_loss_weight
                loss['ssim_scene'] = (1 - self.ssim(scene_renders['img'][cap_idx], data['img'][cap_idx], mask=1-data['mask'][cap_idx])) * cfg.ssim_loss_weight

      
            reg_scale = 1.0 if cap_idx.numel() > 0 else cfg.gen_rgb_loss_weight
            active_mask = human_assets['active_mask'].detach()          # (B,V,1)
            vis_scale = active_mask.numel() / active_mask.sum().clamp(min=1.0)
            gating_live = self.human_gaussian.visibility_active(
                use_vis, cur_itr if mode == 'train' else None)
            vis_mask = active_mask if gating_live else None
            ...
            loss['gaussian_mean_reg'] = (human_offsets['mean_offset'] ** 2 + human_offsets['mean_offset_offset'] ** 2) * weight * active_mask * vis_scale
            ...
            if cfg.is_warmup:
                loss['gaussian_scale_reg'] = (human_assets['scale_wo_clamp'] ** 2 + human_offsets['scale_offset'] ** 2) * weight * active_mask * vis_scale
            else:
                loss['gaussian_scale_reg'] = (human_assets['scale'] ** 2 + human_offsets['scale_offset'] ** 2) * weight * active_mask * vis_scale

            weight = torch.ones((1,smpl_x.vertex_num_upsampled,1)).float().cuda()
            weight[:,self.human_gaussian.is_face_expr,:] = 50
            weight[:,self.human_gaussian.is_cavity,:] = 0.1
            loss['lap_mean'] = (self.lap_reg(mesh_neutral_pose[None,:,:].detach() + human_offsets['mean_offset'], mesh_neutral_pose[None,:,:].detach(), vis_mask) + \
                                self.lap_reg(mesh_neutral_pose[None,:,:].detach() + human_offsets['mean_offset'] + human_offsets['mean_offset_offset'], mesh_neutral_pose[None,:,:].detach(), vis_mask)) * 100000 * weight * active_mask * vis_scale
            weight = torch.ones((1,smpl_x.vertex_num_upsampled,1)).float().cuda() * 10 
            weight[:,self.human_gaussian.is_rhand,:] = 10
            weight[:,self.human_gaussian.is_lhand,:] = 10
            weight[:,self.human_gaussian.is_face_expr,:] = 0
            loss['lap_scale'] = (self.lap_reg(human_assets['scale'], None, vis_mask) + self.lap_reg(human_assets_refined['scale'], None, vis_mask)) * 100000 * weight * active_mask * vis_scale
            ...
            loss['lap_rgb'] = (self.lap_reg(human_assets['rgb'], None, vis_mask) + self.lap_reg(human_assets_refined['rgb'], None, vis_mask)) * weight * active_mask * vis_scale
            ...
            if cur_itr >= cfg.rgb_offset_start_iter:
                ramp2 = min(1.0, (cur_itr - cfg.rgb_offset_start_iter) / max(1, cfg.rgb_offset_ramp_len))
                rgb_off = human_offsets['rgb_offset']
                w = torch.ones((1, smpl_x.vertex_num_upsampled, 1), device=rgb_off.device)
                w[:, self.human_gaussian.is_face, :] = cfg.rgb_offset_face_w
                w[:, self.human_gaussian.is_lhand, :] = cfg.rgb_offset_hands_w
                w[:, self.human_gaussian.is_rhand, :] = cfg.rgb_offset_hands_w
                w_rest = cfg.rgb_offset_rest_w
                w[:, ~(self.human_gaussian.is_face | self.human_gaussian.is_lhand | self.human_gaussian.is_rhand), :] = w_rest

                loss['rgb_offset_l2']  = ramp2 * cfg.rgb_offset_l2_weight  * (rgb_off ** 2 * w * active_mask).mean() * vis_scale
                zeros_ref = torch.zeros_like(rgb_off)
                loss['rgb_offset_lap'] = ramp2 * cfg.rgb_offset_lap_weight * self.lap_reg(rgb_off, zeros_ref, vis_mask) * active_mask * vis_scale

            if reg_scale != 1.0:
                for key in ['gaussian_mean_reg', 'gaussian_mean_hand_reg', 'gaussian_scale_reg',
                            'lap_mean', 'lap_scale', 'lap_rgb', 'hand_rgb_reg', 'arm_rgb_reg',
                            'joint_offset_reg', 'joint_offset_sym_reg', 'rgb_offset_l2', 'rgb_offset_lap']:
                    if key in loss:
                        loss[key] = loss[key] * reg_scale

            
            if cur_itr % 100 == 0:
                import os, os.path as osp
                from torchvision.utils import save_image
                pred_image = human_renders_refined["img"]  
                if cur_itr >= 1:
                    out_dir = osp.join(cfg.output_dir, 'debug_rendered', cfg.subject_id)
                    os.makedirs(out_dir, exist_ok=True)
                    save_image(pred_image.detach(), osp.join(out_dir, f'rendered_image_{cur_itr:06d}.png'))
    ...
    def adjust_gaussians(self, track, gaussian, cur_itr, optimizer):
        if track['mean_2d_grad'] is not None:
