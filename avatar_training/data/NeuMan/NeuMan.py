...
class NeuMan(torch.utils.data.Dataset):
    ...
    def load_generated_data(self):
        ...
        gen_idx_list = []
        root_path = osp.join('..', 'data', 'NeuMan', 'data', cfg.subject_id + '_gen')
        if not osp.isdir(root_path):
            print('[gen] skip missing block: ' + root_path)
            return
        offset = cfg.gen_id_offset_base

        img_paths = {int(x.split('/')[-1][:-4]): x for x in glob(osp.join(root_path, 'images', '*.png'))}
        ...
        smplx_paths = {int(x.split('/')[-1][:-5]): x for x in glob(osp.join(root_path, 'smplx_optimized', 'smplx_params', '*.json'))}

        local_idxs = sorted(set(img_paths) & set(mask_paths) & set(cam_paths) & set(kpt_paths) & set(smplx_paths))
        ...
        gen_repeat = int(round(self.GEN_FRAC / max(1e-8, 1 - self.GEN_FRAC) * n_cap / n_gen))
        gen_repeat = max(gen_repeat, 1)
        self.frame_idx_list = self.frame_idx_list + gen_idx_list * gen_repeat
        self.num_generated = n_gen * gen_repeat
        print('[gen] captured %d + generated %d x %d = %d (generated fraction %.3f)' % (
            n_cap, n_gen, gen_repeat, len(self.frame_idx_list), self.num_generated / len(self.frame_idx_list)))
