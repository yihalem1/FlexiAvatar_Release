...
class TalkShow(torch.utils.data.Dataset):
    ...
    SUBJECTS = list(TRAIN_REPEAT)

    GEN_FRAC = 0.2

    def __init__(self, transform, data_split):
        assert cfg.subject_id in self.SUBJECTS, \
            "'%s' is not a {0} subject. Available: {1}".format(
                type(self).__name__, ', '.join(self.SUBJECTS)) % cfg.subject_id
        self.train_repeat = self.TRAIN_REPEAT[cfg.subject_id]
        self.data_split = data_split
        self.root_path = osp.join('..', 'data', 'TalkShow', 'data', cfg.subject_id)
        ...
        self.captured_cam_idxs = list(self.cam_params.keys())
        self.cam_dist = self.get_cam_dist()

        self.source_flag = {frame_idx: 0 for frame_idx in set(self.frame_idx_list)}
    ...
    def load_generated_data(self):
        self.num_captured = len(self.frame_idx_list)
        self.num_generated = 0
        if (self.data_split != 'train') or cfg.fit_pose_to_test:
            return

        gen_idx_list = []
        root_path = osp.join('..', 'data', 'TalkShow', 'data', cfg.subject_id + '_gen')
        if not osp.isdir(root_path):
            return
        offset = cfg.gen_id_offset_base

        img_paths = {int(x.split('/')[-1][:-4]): x for x in glob(osp.join(root_path, 'frames', '*.png'))}
        mask_paths = {int(x.split('/')[-1][:-4]): x for x in glob(osp.join(root_path, 'masks', '*.png'))}
        cam_paths = {int(x.split('/')[-1][:-5]): x for x in glob(osp.join(root_path, 'cam_params', '*.json'))}
        kpt_paths = {int(x.split('/')[-1][:-5]): x for x in glob(osp.join(root_path, 'keypoints_whole_body', '*.json'))}

        # match the captured loader's preference for the smoothed fit
        smplx_dir = osp.join(root_path, 'smplx_optimized', 'smplx_params_smoothed')
        if not osp.isdir(smplx_dir):
            smplx_dir = osp.join(root_path, 'smplx_optimized', 'smplx_params')
        smplx_paths = {int(x.split('/')[-1][:-5]): x for x in glob(osp.join(smplx_dir, '*.json'))}


        available = set(img_paths) & set(mask_paths) & set(cam_paths) & set(kpt_paths) & set(smplx_paths)


        split_path = osp.join(root_path, 'frame_list_train.txt')
        if osp.isfile(split_path):
            with open(split_path) as f:
                listed = set(int(x) for x in f.readlines() if x.strip())
            local_idxs = sorted(available & listed)
            print('[gen] %d listed, %d with all inputs -> %d usable' % (
                len(listed), len(available), len(local_idxs)))
        else:
            local_idxs = sorted(available)
            print('[gen] %d/%d frames usable' % (len(local_idxs), len(img_paths)))

        for local_idx in local_idxs:
            frame_idx = local_idx + offset
            self.img_paths[frame_idx] = img_paths[local_idx]
            self.mask_paths[frame_idx] = mask_paths[local_idx]
            with open(cam_paths[local_idx]) as f:
                self.cam_params[frame_idx] = {k: np.array(v, dtype=np.float32) for k, v in json.load(f).items()}
            with open(kpt_paths[local_idx]) as f:
                self.kpts[frame_idx] = np.array(json.load(f), dtype=np.float32)
            with open(smplx_paths[local_idx]) as f:
                self.smplx_params[frame_idx] = {k: torch.FloatTensor(v) for k, v in json.load(f).items()}
            self.source_flag[frame_idx] = 1
            gen_idx_list.append(frame_idx)

        if len(gen_idx_list) == 0:
            return


        n_cap, n_gen = len(self.frame_idx_list), len(gen_idx_list)
        target = self.GEN_FRAC / max(1e-8, 1 - self.GEN_FRAC) * n_cap
        gen_repeat = int(round(target / n_gen))
        if gen_repeat >= 1:
            gen_samples = gen_idx_list * gen_repeat
            how = 'x%d' % gen_repeat
        else:
            keep_n = max(1, int(round(target)))
            step = n_gen / float(keep_n)
            gen_samples = [gen_idx_list[int(i * step)] for i in range(keep_n)]
            how = 'subsampled to %d' % keep_n
        self.frame_idx_list = self.frame_idx_list + gen_samples
        self.num_generated = len(gen_samples)
        print('[gen] captured %d + generated %d %s = %d (generated fraction %.3f)' % (
            n_cap, n_gen, how, len(self.frame_idx_list), self.num_generated / len(self.frame_idx_list)))
    ...
    def load_data(self):
        ...
        if self.data_split == 'train':
            frame_idx_list *= self.train_repeat
    ...
    def __getitem__(self, idx):
        ...
        data = {'img': img, 'mask': mask, 'bbox': bbox, 'cam_param': cam_param, 'frame_idx': frame_idx,
                'source': self.source_flag.get(frame_idx, 0)} # 0: captured, 1: generated
        return data

    def get_cam_dist(self):
        if self.scene_from_colmap:
            cam_pos_list = []
            for frame_idx in self.captured_cam_idxs:
