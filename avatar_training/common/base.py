...
def report_visibility_state(model, logger):
  
    hg = getattr(model, 'human_gaussian', None)
    if hg is None or not hasattr(hg, 'vis_ready'):
        return
    if hg.vis_ready.item() == 0:
        logger.info('[vis] checkpoint carries no visibility mask -> gating disabled for this run')
    elif hg.vis_enabled.item() == 0:
        logger.info('[vis] checkpoint mask present but gating was disabled during training '
                    '(fully observed sequence) -> rendering all Gaussians')
    else:
        tau, eta, mu0 = hg.vis_stats.tolist()
        kept, total = int(hg.vis_keep.sum()), hg.vis_keep.numel()
        logger.info('[vis] mask loaded: tau*=%.3f eta=%.3f mu0=%.4f | rendering %d/%d Gaussians (%.1f%% pruned)'
                    % (tau, eta, mu0, kept, total, (1 - kept / total) * 100))
...
class Trainer(Base):
    ...
    def _make_model(self, epoch=None):
        if cfg.continue_train:
            ...
            report_visibility_state(model.module, self.logger)
        else:
