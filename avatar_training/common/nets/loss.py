...
class LaplacianReg(nn.Module):
    ...
    def compute_laplacian(self, x, neighbor_idxs, neighbor_weights, active_mask=None):
        if active_mask is None:
            lap = x + (x[:, neighbor_idxs] * neighbor_weights[None, :, :, None]).sum(2)
            return lap

        neighbor_x = x[:, neighbor_idxs]                                 # (B,V,K,C)
        neighbor_mask = active_mask[:, neighbor_idxs]                    # (B,V,K,1)
        valid_edge = (neighbor_weights < 0).float()[None, :, :, None]    # (1,V,K,1)
        effective = valid_edge * neighbor_mask
        count = effective.sum(2)                                         # (B,V,1)
        neighbor_mean = (neighbor_x * effective).sum(2) / count.clamp(min=1e-6)
        has_neighbors = (count > 0).float()
        return (x - neighbor_mean) * has_neighbors

    def forward(self, out, target, active_mask=None):
        if target is None:
            lap_out = self.compute_laplacian(out, self.neighbor_idxs, self.neighbor_weights, active_mask)
            loss = lap_out ** 2
            return loss
        else:
            lap_out = self.compute_laplacian(out, self.neighbor_idxs, self.neighbor_weights, active_mask)
            lap_target = self.compute_laplacian(target, self.neighbor_idxs, self.neighbor_weights, active_mask)
