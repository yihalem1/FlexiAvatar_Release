...
class NeuMan(torch.utils.data.Dataset):
    def __init__(self, transform):
        ...
        self.cam_param, self.img_paths, self.kpts, self.smplx_params, self.flame_params, self.flame_shape_param, self.frame_idx_list = self.load_data()
    ...
    def load_data(self):
        ...
        focal = np.array((focal_x, focal_y), dtype=np.float32) # shared across all frames
        princpt = np.array((princpt_x, princpt_y), dtype=np.float32) # shared across all frames
    ...
    def get_smplx_trans_init(self):
        for i in range(len(self.frame_idx_list)):
            ...
            focal, princpt = self.cam_param['focal'], self.cam_param['princpt']
    ...
    def __getitem__(self, idx):
        ...
        focal = self.cam_param['focal']
        princpt = self.cam_param['princpt']
