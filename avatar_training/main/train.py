    ...
    for epoch in range(trainer.start_epoch, cfg.end_epoch):
        ...
        for itr, data in enumerate(trainer.batch_generator):
            ...
            if (not cfg.fit_pose_to_test) and (cur_itr < cfg.densify_end_itr):
                with torch.no_grad():
                    grads = [x.grad for x in stats['mean_2d']]
                    stats['mean_2d_grad'] = torch.stack([g.detach() for g in grads]) if all(g is not None for g in grads) else None
