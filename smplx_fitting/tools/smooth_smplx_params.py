...
render_mesh_save_path = osp.join(root_path, 'smplx_optimized', 'render_smooth_mesh')
...
os.makedirs(render_mesh_save_path, exist_ok=True)
...
for frame_idx in tqdm(frame_idx_list):
    ...
    # render (mesh overlaid on the image)
    ...
    # render (mesh only, on a blank white background, no image overlay)
    blank_bkg = np.full_like(img, 255)
    render_mesh_only = render_mesh(vert.numpy(), smpl_x.face, {'focal': cam_param['focal'], 'princpt': cam_param['princpt']}, blank_bkg, 1.0)
    cv2.imwrite(osp.join(render_mesh_save_path, str(frame_idx) + '_smplx.jpg'), render_mesh_only)
