...
def main():
    ...
    log_interval = 500
    total_opt_steps = 0
    for _epoch in range(cfg.end_epoch):
        cfg.set_itr_opt_num(_epoch)
        total_opt_steps += cfg.itr_opt_num * trainer.itr_per_epoch
    global_opt_step = 0
    avg_step_time = None
    ...
    for epoch in range(cfg.end_epoch):
        ...
        for itr_data, data in enumerate(trainer.batch_generator):
            ...
            for itr_opt in range(cfg.itr_opt_num):
                ...
                if avg_step_time is None:
                    ...
                    avg_step_time = 0.95 * avg_step_time + 0.05 * step_time

                # log
                should_log = (itr_opt % log_interval == 0) or (itr_opt == cfg.itr_opt_num - 1)
                if should_log:
                    elapsed_time = time.time() - train_start_time
                    remaining_steps = max(total_opt_steps - global_opt_step, 0)
                    eta_time = remaining_steps * avg_step_time
                    est_total_time = elapsed_time + eta_time
                    progress = 100.0 * global_opt_step / max(total_opt_steps, 1)

                    screen = [
                        'epoch %d/%d itr_data %d/%d itr_opt %d/%d:' % (epoch, cfg.end_epoch, itr_data, trainer.itr_per_epoch, itr_opt, cfg.itr_opt_num),
                        'lr: %g' % (trainer.get_lr()),
                        'global_step: %d/%d' % (global_opt_step, total_opt_steps),
                        'step_time: %.3fs' % (step_time),
                        'elapsed: %s' % (sec_to_hms(elapsed_time)),
                        'eta: %s' % (sec_to_hms(eta_time)),
                        'est_total: %s' % (sec_to_hms(est_total_time)),
        ...
        elapsed_time = time.time() - train_start_time
        remaining_steps = max(total_opt_steps - global_opt_step, 0)
        eta_time = remaining_steps * (avg_step_time if avg_step_time is not None else 0.0)
        print([
            'epoch %d/%d summary:' % (epoch, cfg.end_epoch),
            'epoch_time: %s' % (sec_to_hms(epoch_elapsed_time)),
            'elapsed: %s' % (sec_to_hms(elapsed_time)),
            'eta: %s' % (sec_to_hms(eta_time)),
            'est_total: %s' % (sec_to_hms(elapsed_time + eta_time))
        ])
    ...
    for frame_idx in frame_idx_list:
        ...
        video_out.write(out)
    video_out.release()
    print(['training_done', 'total_time: %s' % (sec_to_hms(time.time() - train_start_time))])
